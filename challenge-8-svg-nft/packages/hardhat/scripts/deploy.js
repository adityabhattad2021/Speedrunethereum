...
const main = async () => {
  ...
  if (network.name === "localhost") {
    const ethUSDAggerator = await deploy("MockV3Aggregator",[DECIMALS,INITIAL_PRICE]);
    ethUSDPriceFeedAddress=ethUSDAggerator.address;
  }else{
    // Sepolie Price Feed Address.
    ethUSDPriceFeedAddress='0x694AA1769357215DE4FAC081bf1f309aDC325306';
  }

  // deploy the contract with all the artworks forSale
  const yourCollectible = await deploy("YourCollectible",[ethUSDPriceFeedAddress] /*,[ bytes32Array ]*/); // <-- add in constructor args like line 19 vvvv
