  ...
  function recieveVoucher(clientAddress) {
    ...
    async function processVoucher(voucher) {
      ...
      */
      ...
      // recreate the packed, hashed, and arrayified message from reimburseService
      const packed = ethers.utils.solidityPack(["uint256"],[updatedBalance]);
      const hashed = ethers.utils.keccak256(packed);
      const arrayified = ethers.utils.arrayify(hashed);
      ...
      // use ethers.utils.verifyMessage() to confirm that voucher signer was `clientAddress`
      const signer = ethers.utils.verifyMessage(arrayified, voucher.data.signature);
   
      // console.log(`SIGNER IS: ${JSON.stringify(voucher.data)}`);
      if (signer !== clientAddress) {
        console.error(`Invalid voucher signature for client ${clientAddress} ${signer}`);
        logVouchers();
        return;
      }
